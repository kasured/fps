 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum function on empty list is 0") {
    assert(sum(Nil) === 0)
  }

  test("max function on empty list should throw exception") {
  ...
  test(s"sum of all numbers in List $testList is 20") {
    assert(sum(testList) === 20)
  }

  val anotherTestList = List(-1, -1, -2, -3, 0, 8)
  test(s"max of all numbers in List $anotherTestList is 8") {
    assert(max(anotherTestList) === 8)
  }
